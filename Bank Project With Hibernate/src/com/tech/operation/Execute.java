...
public class Execute {
	...
	List<AccountTransaction> l = new ArrayList<>();
	...
	public void createAccount() {
		...
		tx.setDate(new Date());
		tx.setDebitorCredit("Credit");
		tx.setDescription("Credit Amount");
		tx.setBalance(ac.getDepositMoney());
		tx.setAccount(ac);
		// List<AccountTransaction> l=new ArrayList<>();
		l.add(tx);
		ac.setAccTransaction(l);
	...
	public void depositMoney() {
		...
		if (ac1 != null) {
			...
			if (p != 0) {
				...
				System.out.println("Total Account Balance: " + total_balance);
...
// By using this method we can withdraw amount from our bank account
			...
			if (amount > 0) {
				...
				if (total >= 500) {
					...
					if (h != 0) {
						...
						tx.setAmount(amount);
						tx.setBalance(total);
						tx.setDebitorCredit("Debit");
						tx.setDescription("Amount Debited from Account");
						tx.setAccount(ac1);
			...
			if (ac2 != null) {
				...
				if (sendMoney > 0) {
					...
					if (total >= 500) {
						...
						tx1.setDescription("Amount Debited from your Account");
						tx1.setDebitorCredit("Debit");
						tx1.setAmount(sendMoney);
						...
						// transection
						tx.setDate(new Date());
						tx.setDescription("Amount Credited in your Account");
						tx.setDebitorCredit("Credit");
						tx.setAmount(sendMoney);
    ...
    //to get transaction from account
	public void getTransaction() {
		...
		System.out.println("Enter your Aadhar number:");
		long aadhar = sc.nextLong();
		Account ac1 = sn.get(Account.class, aadhar);
		if (ac1 != null) {
			System.out.println("Aadhar Number: " + ac1.getAadharNumber());
			System.out.println("Transaction Details:");
			System.out.println();
			for (AccountTransaction t : ac1.getAccTransaction()) {
				System.out.println(t.getDate());
				System.out.println(t.getAmount());
				System.out.println(t.getDebitorCredit());
