...
public class Execute {
	...
	public void depositMoney() {
		...
		if (ac1 != null) {
			...
			Query<Account> hql = sn.createQuery("update Account set depositMoney=:amount where accountNumber=:accno");
...
// By using this method we can withdraw amount from our bank account
			...
			if (amount > 0) {
				...
				if (total >= 500) {
					Query<Account> hql = sn
							.createQuery("update Account set depositMoney=:amount where accountNumber=:accno");
	...
	public void transferMoney() {
		...
		System.out.println("Enter Account Number for TransferMoney:");
		long acc1 = sc.nextLong();
		Account ac1 = sn.get(Account.class, acc1);
		if (ac1 != null) {
			System.out.println("Enter Account number from TransferMoney:");
